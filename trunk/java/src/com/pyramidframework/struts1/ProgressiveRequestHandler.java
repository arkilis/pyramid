...
import com.pyramidframework.upload.MonitoredDiskFileItemFactory;
import com.pyramidframework.upload.UploadListener;
import com.pyramidframework.upload.UploadProgress;

public class ProgressiveRequestHandler implements MultipartRequestHandler {
	...
	public void handleRequest(HttpServletRequest request) throws ServletException {
		...
		//标记完成
		UploadProgress progress = (UploadProgress)request.getSession().getAttribute(UploadProgress.UPLOAD_ATTRIBUTE_KEY);
		listener.bytesRead(progress.getTotalSize() - progress.getBytesRead());
	}
	...
	public void rollback() {
		Iterator iterator = elementsFile.values().iterator();
	 ...
	 * 'G', representing kilobytes, megabytes and gigabytes respectively. <p/>
	...
	static class CommonsFormFile implements FormFile, Serializable {
		...
		private static final long serialVersionUID = -5198593246505973014L;
		/**
		...
		/**
		 * Sets the size, in bytes, for this file. <p/> NOTE: This method is not
		 * supported in this implementation.
		 * 
		 * @param filesize
